...
if (!inputPath) {
  console.error('Usage: npm run pdf-to-md -- <pdf-path> [--slug=name]');
  process.exit(1);
}
...
const slug = slugArg ?? path.basename(fileName, path.extname(fileName)).replace(/[^a-z0-9-]/gi, '-').toLowerCase();
const outPath = path.join(SOURCES_DIR, `${slug}.md`);

main().catch((err) => {
  console.error('[pdf-to-md] 失败：', err.message);
  process.exit(1);
});
...
async function main() {
  ...
  if (!usePrecision && sizeMB > 10) {
    throw new Error(`Agent API 限 10MB，本文件 ${sizeMB.toFixed(2)} MB。配 MINERU_API_KEY 切 Precision API。`);
  }
...
async function uploadAndStartTask(name: string, buf: Buffer): Promise<string> {
  ...
  const initData = (await initRes.json()) as { task_id?: string; file_url?: string; data?: { task_id?: string; file_url?: string } };
  ...
  const uploadRes = await fetch(uploadUrl, { method: 'PUT', body: buf });
  if (!uploadRes.ok) {
