# AGENTS.md — sbj-website

> 项目级指令。Codex 每次进入此目录会自动加载。
> 全局指令见 `~/.Codex/AGENTS.md`，编程项目通用规则见 `D:\workspace\01_项目-Coding\AGENTS.md`。
...
5. **`C:\Users\admin\.Codex\plans\1-giggly-cat.md`** — V8 plan 主文档（设计思路、风险清单、商业模式、autoplan review 记录）
...
- **AI 模型用国产合规**（DeepSeek 主 + 豆包/讯飞备）；不用 Codex/GPT/Gemini
